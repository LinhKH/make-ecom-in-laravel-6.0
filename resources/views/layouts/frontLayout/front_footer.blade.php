		...
		<div class="container">
			<div class="row">
				...
				<div class="col-sm-3 col-sm-offset-1">
					<div class="single-widget">
						<h2>About Shopper</h2>
						<form action="javascript:void(0);" type="post" class="searchform">
							<input type="hidden" name="token" value="{{ csrf_token() }}" >
							<input onfocus="enableSubscriber();" onfocusout="checkSubscriber();" type="email" placeholder="Your email address" required name="subscriber_email" id="subscriber_email"/>
							...
							<div id="statusSubscriber" style="display: none; color:red"></div>
		...
		$.ajax({
			...
			success:function(resp) {
				if(resp=='exists') {
					...
					$("#btnSubmit").hide();
					$("#statusSubscriber").html("Subscriber email already exists");
				}
			},error:function() {
				alert("Error")
			}
		...
		$.ajax({
			...
			success:function(resp) {
				if(resp=='exists') {
					...
					$("#statusSubscriber").show();
					$("#statusSubscriber").css({"color":"green"}).html("Thanks for Subscribing !");
				}
			},error:function() {
				alert("Error")
			}
