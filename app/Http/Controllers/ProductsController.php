...
class ProductsController extends Controller
{
    ...
    public function addtocart(Request $request)
    {
        ...
        if (!empty($data['wishListButton']) && $data['wishListButton'] == 'Wish List') {
            ...
            if (!Auth::check()) {
                return redirect()->back()->with('flash_message_error', 'Please logged in to add product in your Wish List');
            }
            // Check size is selected
            if (empty($data['size'])) return redirect()->back()->with('flash_message_error', 'Please select size to add product in your Wish List');
            ...
            $product_price = isset($proPrice['price']) ? $proPrice['price'] : 0;
            ...
            $wishListCount = DB::table('wish_list')->where(['product_id' => $data['product_id'], 'user_email' => $user_email, 'product_color' => $data['product_color'], 'size' => $product_size])->count();
            if ($wishListCount > 0) {
                ...
                DB::table('wish_list')->insert([
                    ...
                    'price' => $product_price, 'size' => $product_size, 'quantity' => $quantity, 'user_email' => $user_email, 'created_at' => $created_at
                ]);
    ...
    public function wishList()
    {
        ...
        /*echo "<pre>"; print_r($userWishList); die;*/
    ...
    public function delWishListProduct($id)
    {
        DB::table('wish_list')->where(['id' => $id])->delete();
        return redirect()->back()->with('flash_message_success', 'Product has been deleted in Wish List!');
    }
