...
class UsersController extends Controller
{
    ...
    public function viewUsersCharts(){
        ...
        return view('admin.users.view_users_charts')->with(compact('current_month_users','last_month_users','last_to_month_users'));
    }

    public function viewUsersCountryCharts(){
        $getUsersContries = User::select('country as label', DB::raw('count(country) as y'))->groupBy('country')->get()->toArray();

        // dd($getUsersContries);
        foreach ($getUsersContries as $key => $value) {
            if($value['label'] == '') {
                $getUsersContries[$key]['label'] = 'Other';
            }
        }
        
        return view('admin.users.view_users_country_charts')->with(compact('getUsersContries'));
    }
}
